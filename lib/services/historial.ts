...
async function _guardarPG(input: GuardarCotizacionInput): Promise<void> {
  ...
  const cotRows = await sql<{ id_cotizacion: number }[]>`
    INSERT INTO cotizacion (
      numero_cotizacion,
      id_broker, id_unidad, id_condicion,
      valor_uf_snapshot, pie_pct, n_cuotas_pie, plazo_anios, plusvalia_anual_pct,
      descuento_pct_snapshot, bono_pie_pct_snapshot,
      cuoton_pct_snapshot, pie_periodo_constr_pct_snapshot, pie_credito_directo_pct_snapshot,
      reserva_clp_snapshot, modalidad_pago_snapshot,
      precio_lista_uf, precio_venta_uf, tasacion_uf,
      pie_total_uf, total_pie_inmob_uf, ch_final_uf
    ) VALUES (
      ${input.numero},
      ${idBroker}, ${id_unidad}, ${id_condicion ?? null},
      ${r.valorUF}, ${input.piePct}, ${r.cuotasPieN}, ${input.plazoAnios}, ${input.plusvaliaAnual},
      ${input.unidad.descuento}, ${input.unidad.bonoPie},
      ${input.unidad.cuoton}, ${input.unidad.piePeriodoConstruccion}, ${input.unidad.pieCreditoDirecto},
      ...
      ${r.precioListaTotal}, ${r.valorVentaUF}, ${r.tasacionUF},
      ${r.pieTotalUF}, ${r.totalPieInmobUF}, ${r.creditoHipFinalUF}
    )
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await sql`
      INSERT INTO cotizacion_escenario (
        id_cotizacion, numero_escenario, cae,
        arriendo_clp, cuota_mensual_clp, cuota_mensual_uf,
        flujo_mensual_clp, flujo_acum_clp,
